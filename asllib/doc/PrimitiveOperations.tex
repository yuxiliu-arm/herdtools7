...
\\
        ...
        & \hypertarget{ast-plus}{} \hypertarget{ast-minus}{} \hypertarget{ast-or}{} \hypertarget{ast-xor}{} \hypertarget{ast-and}{} \hypertarget{ast-bvconcat}{}
\\
...
\begin{itemize}
  ...
  \begin{itemize}
    \item $\vvone$ is a bitvector literal for $a_{1..j}$;
    \item $\vvtwo$ is a bitvector literal for $b_{1..k}$;
    \item $\op$ is $\BVCONCAT$;
    \item define $c_i$ as $a_i$ if $1 \leq i \leq j$ and $b_{i-j}$ otherwise, for $i=1..(j+k)$;
    \item define $\vr$ as the bitvector literal for $c$.
  \end{itemize}
...
\inferrule[concat\_bits]{
  i=1..j: c_i=a_i\\
  i=(j+1)..(j+k): c_i=b_{i-j}
}{
  \binopliterals(\overname{\BVCONCAT}{\op}, \overname{\lbitvector(a_{1..j})}{\vvone}, \overname{\lbitvector(b_{1..k})}{\vvtwo}) \typearrow \lbitvector(c_{1..(j+k)})
}
\end{mathpar}
