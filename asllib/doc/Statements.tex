...
\begin{itemize}
  ...
  \item annotating each expression $\vargs_i$ in $\tenv$ yields $(\vt_i, \vargsp_i)$\ProseOrTypeError, for $i=1..k$;
  \item checking that $\vt_i$ is a singular type yields $\True$\ProseOrTypeError, for each $i=1..k$;
...
\begin{mathpar}
  \inferrule{%
    \vi\in\listrange(\vargs): \annotateexpr{\vargs[i], \tenv} \typearrow (\vt_i, \vargsp[i])\OrTypeError \\
    \vi\in\listrange(\vargs): \checktrans{\issingular(\vt_i)}{\BadPrintType} \typearrow \True \OrTypeError \\
  }{%
    \annotatestmt (\tenv, \overname{\SPrint(\vargs, \vnewline)}{\vs}) \typearrow (\SPrint(\vargsp, \vnewline), \tenv)
  }
\end{mathpar}
