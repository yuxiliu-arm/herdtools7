...
--- @return string
local function tex2mml(s, isDisplay)
  if isDisplay then
    s = "=====" .. s
  end
  -- logging.temp(s)
  local res = pandoc.pipe("nc", { "localhost", "3000" }, s)
  -- logging.temp(res)
  -- logging.temp(".")
  return res
end
  ...
  if res:find("prooftree") then
    ...
    if res:find("\\and%s") then
      ...
      for _, e in pairs(elems) do
        table.insert(blocks, pandoc.RawInline("html", tex2mml(e.text, true)))
      end

      return blocks
    end
  end
  return pandoc.RawInline("html", tex2mml(res, elem.mathtype == "DisplayMath"))
end
